...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // Return str1 without the overlapping part at the end plus str2
        // This code remove one of the duplicate part
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        // Set up a boolean variable to check the precondition
        boolean checkPre = true;
        // For loop to see if str is a substring of any s in strSet
        // If str is the substring of strSet, then it will not run the second loop
        for (String s : strSet) {
            if (s.indexOf(str) >= 0) {
                checkPre = false;
            }
        ...
        // If it satisfy the precondition
        if (checkPre) {
            // Create a new temporary set to hold filtered elements
            Set<String> temp = strSet.newInstance();
            // Check each element in strSet whether it is a substring of str
            // If it is, then do not add that element to temp
            for (String s : strSet) {
                if (str.indexOf(s) == -1) {
                    temp.add(s);
                }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Create a new set to hold the result lines
        Set<String> strSet = new Set1L<>();

        // Loop through the input until end of stream (EOS)
        while (!input.atEOS()) {
            // Read the next line from input
            String str = input.nextLine();
            // Avoid adding substrings
            addToSetAvoidingSubstrings(strSet, str);
        }
        // Return the final set of filtered strings
        return strSet;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            // If it is, then replace '~' to a line separator
            if (text.charAt(i) == '~') {
                out.println();
            } else {
                // If it is not, then print out letter
