...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        NaturalNumber lowEnough = new NaturalNumber2(0); // Initialize lowEnough to zero
        // Initialize tooHigh and then to n + 1
        NaturalNumber tooHigh = new NaturalNumber2(n);
        tooHigh.increment();
        NaturalNumber difference = new NaturalNumber2(tooHigh); // Initialize difference
        difference.subtract(lowEnough);
        // Iteration until tooHigh = lowEnough + 1
        while (difference.compareTo(new NaturalNumber2(1)) > 0) {
            // Use Interval Halving method and create mid, powerMid
            NaturalNumber mid = new NaturalNumber2(lowEnough);
            mid.add(tooHigh);
            mid.divide(new NaturalNumber2(2));
            NaturalNumber powerMid = new NaturalNumber2(mid);
            powerMid.power(r);
            if (powerMid.compareTo(n) > 0) {
                tooHigh.copyFrom(mid); // Update tooHigh
            } else {
                lowEnough.copyFrom(mid); // Update lowEnough
            }
            difference.copyFrom(tooHigh);
            difference.subtract(lowEnough); // Update difference
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numbers.length; i++) { // Iteration using for loop
            NaturalNumber n = new NaturalNumber2(numbers[i]);
            NaturalNumber r = new NaturalNumber2(results[i]);
            root(n, roots[i]);
            if (n.equals(r)) { // Check whether it pass the test
