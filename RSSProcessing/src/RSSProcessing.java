...
public final class RSSProcessing {
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        if (titleIndex != -1 && item.child(titleIndex).numberOfChildren() > 0) {
            titleOrDesc = item.child(titleIndex).child(0).label();
            out.println("Title: " + titleOrDesc);
        } else if (descIndex != -1 && item.child(descIndex).numberOfChildren() > 0) {
            titleOrDesc = item.child(descIndex).child(0).label();
            out.println("Description: " + titleOrDesc);
        }
        String link = "";
        ...
        if (pubDateIndex != -1) {
            ...
            out.println("Publication Date: " + pubDate);
        } else {
            out.println("Publication Date: is missing.");
        }
        String url = "";
        int sourceIndex = getChildElement(item, "source");
        if (sourceIndex != -1) {
            url = item.child(sourceIndex).attributeValue("url");
            out.println("Source URL: " + url);
            if (item.child(sourceIndex).numberOfChildren() > 0) {
                out.println("Source: " + item.child(sourceIndex).child(0).label());
            }
        } else {
            out.println("Source: is missing.");
        }
    ...
    public static void main(String[] args) {
        ...
        out.println("Description: " + channel.child(descriptionIndex).child(0).label());
